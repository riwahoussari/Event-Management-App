  ...
  return (
    <>
      <header className="w-full px-6 py-4 border-b shadow-sm flex justify-between items-center">
        ...
        {/* Right profile link */}
        {user && user.account_type !== "admin" && <ProfilePic id={user.id} />}
      </header>
      <Outlet />
    </>
  );
...
function DropdownNavMenu({ user }: { user: UserType | null }) {
...
function ProfilePic({ id }: { id: number }) {
  const { data: user } = useFetch<UserType>(`/api/users/${id}`);
